...
Two data modes:
  - raw mode:       reads xite_msd.parquet directly (6 features, no user features)
  - processed mode: reads processed/train.parquet from Pipeline 1 (9 features, with user features)

Usage:
    python train_ranking.py configs/stage_b_baseline.yaml
    python train_ranking.py configs/stage_b_lgbm_v4.yaml
"""
...
def load_and_prepare_data(cfg: dict) -> tuple:
    """
    Load parquet data memory-safely using pyarrow iter_batches.

    If cfg["use_processed"] is True (default when data_path points to processed/train.parquet):
      - Reads pre-computed features + label from Pipeline 1 output
      - Features: session_order, video_order, release_year, context_segment,
                  genre_encoded, subgenre_encoded,
                  user_skip_rate, user_favorite_genre_encoded, user_watch_time_avg
    Else (raw mode, points to xite_msd.parquet):
      - Computes features + label from raw XITE data
      - Features: session_order, video_order, release_year, context_segment,
                  genre_encoded, subgenre_encoded
    """
    data_path = cfg["data_path"]
    max_samples = cfg.get("max_samples", 200000)
    use_processed = cfg.get("use_processed", False)

    print(f"[data] Loading {max_samples} rows from {data_path}...")
    print(f"[data] Mode: {'processed (user features)' if use_processed else 'raw (no user features)'}")
    ...
    if use_processed:
        ...
        feature_cols = None  # will be set after encoding
    ...
    if not use_processed:
        ...
        for col in ["session_order", "video_order", "context_segment"]:
